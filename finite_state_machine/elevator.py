...
class ElevatorFSM:
    ...
    async def go_to(self, floor: int):
        try:
            floor = int(floor)
        except ValueError:
            print(f"Incorrect Entry, enter a number between 0 - 11, except {self.__current_floor}")
            return

        if self.__elevator_state != State.Idle:
            pass
        elif (floor < 0 or floor >= self.__floors) or floor == self.__current_floor:
            print("\033[1;31mInvalid Entry\033[0m, Please Try again")
        else:
    ...
    async def _changing_floors(self):
        if self.__elevator_state == State.GoingUp:
            ...
            self.__current_floor -= 1
        print(f"Elevator at Floor \033[34m{self.__current_floor}\033[0m")
...
async def main():
    ...
    print(f"Currently on \033[31m{lift_man.current_floor}\033[0m")
    destination_floor = input("Which floor sir? ")
